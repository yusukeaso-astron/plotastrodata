...
pa.add_text([0.3, 0.3], slist='label')
...
pa.set_axis()
pa.savefig('test2D.png', show=True)
...
pa.add_segment(ampfits=pre+'test2Damp.fits',
               angfits=pre+'test2Dang.fits', skip=3)
pa.add_scalebar(length=50 / 140, label='50 au')
pa.add_text([[0.3, 0.3]], slist=['label'], include_chan=[0,1,2])
...
pa.set_axis(grid={})
pa.savefig('test3D.png', show=True)

# PV case
pa = pad(rmax=0.8, pv=True, vmin=-5, vmax=5)
pa.add_color(fitsimage=pre+'testPV.fits', Tb=True, cblabel='Tb (K)')
pa.add_contour(fitsimage=pre+'testPV.fits', colors='r', sigma=5e-3)
pa.add_text([0.3, 0.3], slist='label')
pa.add_marker([[0.5, 0.5]])
pa.set_axis()
pa.savefig('testPV.png', show=True)
